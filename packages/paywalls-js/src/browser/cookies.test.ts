...
it("readCookie returns null when the cookie does not exist", () => {
  expect(readCookie("missing")).toBeNull();
});

it("writeCookie + readCookie round-trip URL-encoded values", () => {
  writeCookie("_sw_alias_id", "$SuperwallAlias:abc/def+xyz");
  expect(readCookie("_sw_alias_id")).toBe("$SuperwallAlias:abc/def+xyz");
});

it("writeCookie applies SameSite=Lax + Path=/ + Max-Age default", () => {
  writeCookie("k", "v");
...
it("readCookie handles multiple cookies and trims whitespace", () => {
  writeCookie("a", "1");
  writeCookie("b", "2");
  writeCookie("c", "3");
...
it("deleteCookie accepts secure + sameSite + path attributes (P1)", () => {
